"""Backup and recovery procedures for the artist promotion system"""
import os
import shutil
import zipfile
import json
...
class DatabaseBackupManager:
    ...
    def _create_postgres_backup(self, backup_path: Path):
        ...
        if not match:
            raise Exception("Invalid PostgreSQL URL format")
    ...
    def create_manual_backup(self, backup_name: str = None) -> Dict[str, str]:
        ...
        if self.cloud_manager:
            ...
            if "database" in result:
                remote_path = f"backups/db/manual_{timestamp}.sql"
